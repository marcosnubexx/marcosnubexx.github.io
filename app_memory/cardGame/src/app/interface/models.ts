...
const crearColeccionDeCartasInicial = (infoCartas: InfoCarta[]): Carta[] => {
	const infoCartas1 = infoCartas.map((item) => 
    crearCartaInicial(item.idFoto, item.imagen),
  )
	const infoCartas2 = infoCartas.map((item) => 
    crearCartaInicial(item.idFoto, item.imagen),
  )
...
export let tableroInicial: Tablero = crearTableroInicial();